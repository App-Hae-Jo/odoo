...
class VLMProcessor:
    ...
    def _extract_with_ollama(self, image_data, docs_type):
        ...
        try:
            ...
            - 주행거리는 자동차가 지금까지 달린 누적 거리로, 일반적으로 'km' 단위이며 수천에서 수십만 사이의 숫자입니다. 자동차등록증에서는 '검사 유효기간' 표 내부, 오른쪽 열의 '주행거리 확인' 항목에 위치하며, 해당 열의 가장 최근 날짜 기준 숫자만을 주행거리로 추출해야 합니다. 만약 해당 표 또는 열에 주행거리가 명시되어 있지 않다면, 추정하거나 다른 위치의 숫자를 넣지 말고 반드시 빈 문자열("") 또는 null로 표시해야 합니다. 또한 'mm', 'kg', 'cc'와 같은 차량 제원 단위가 붙은 값이나, 전화번호/연도/ID로 보이는 숫자와 혼동하지 말아야 합니다.
